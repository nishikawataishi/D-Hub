...
test("scouts: 承認済み団体のみ作成できる（1〜500文字）", async () => {
  await assertSucceeds(setDoc(doc(as(ORG_VERIFIED), "scouts", "new_scout"), {
    targetUserId: STUDENT, organizationId: ORG_VERIFIED,
    message: "スカウトです", isRead: false,
  }));
  // 審査中団体は不可
  await assertFails(setDoc(doc(as(ORG_PENDING), "scouts", "new_scout2"), {
    targetUserId: STUDENT, organizationId: ORG_PENDING,
    message: "スカウトです", isRead: false,
  }));
  // 501文字は不可
  await assertFails(setDoc(doc(as(ORG_VERIFIED), "scouts", "new_scout3"), {
    targetUserId: STUDENT, organizationId: ORG_VERIFIED,
    message: "あ".repeat(501), isRead: false,
  }));
});
